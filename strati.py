...
# Res2
#¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
paramRes2.Veau = paramRes2.Vres -(paramHXefd.Aext*paramHXefd.Ltot);                          # [m^3] Volume d'eau dans le réservoir --> Calcul Volume EC = (Volume interne du réservoir - Volume des échangeurs de chaleurs)
paramRes2.dV = paramRes2.Veau/paramRes2.nb_y;                                                # [m^3] Volume d'un noeud du réservoir de stockage
paramRes2.deltak = data.kss*(np.pi*paramRes2.De**2/4-paramRes2.As)/paramRes2.As;             # De-stratification due au transfert de chaleur par conduction axiale dans la paroi du réservoir
...
R_condHXecd_wall = np.log(paramHXefd.Do/paramHXefd.Di)/(2*np.pi*data.kcu*paramHXefd.L);      # [K/W] Calcul de la résistance thermique totale au travers de la paroi de l'échangeur EFD
...
for m in range(1,nb_t,1):
    ...
    while erreurTres2[iterTres2] > data.critere:
        ...
        #¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
        for i in range(paramRes2.nb_y+2):  # La boucle se fait sur nb_y+2 afin de calculer les pertes thermiques des parois sur les nb_y noeuds du réservoir ainsi que pour le dessus et le dessous du réservoir (conditions aux frontières)
        ...
        Rpp_pertestopbott[0] = Rpp_convEC[0] + Rpp_condtanktop + Rpp_condiso + Rpp_convair[0];        # [K (m^2)/W] Résistance thermique résultante pour le dessus
        Rpp_pertestopbott[-1] = Rpp_convEC[-1] + Rpp_condtankbott + Rpp_condiso + Rpp_convair[-1]; # [K (m^2)/W] Résistance thermique résultante pour le dessous
        Rpp_pertesside = Rpp_convEC[1:-1] + Rpp_condtankside + Rpp_condiso + Rpp_convair[1:-1]; # [K (m^2)/W] Résistance thermique résultante pour les côtés
        Rpp_pertes = np.vstack((Rpp_pertestopbott[0],Rpp_pertesside,Rpp_pertestopbott[-1]));  # [K (m^2)/W] Vecteur des résistances thermiques totales pour le dessus, dessous et côtés
        ...
        for ii in range(paramHXefd.intlet,paramHXefd.outlet,-1):
            ...
            if paramHXefd.m_in > 0:
                ...
                while erreurTout[iterToutHX] > data.critere:
                    ...
                    TmoyEFD[i,0] = (HXefdresults.Tink[i,m] + HXefdresults.Toutk[i,m])/2; # Calcul de la température moyenne entre la température d'entrée et la température de sortie de l'échangeur
                    ...
                    #
                    R_convHXefd_in[i,0]=(h_convHXefd_in[i,0]*np.pi*paramHXefd.Di*paramHXefd.L)**-1;    # [K/W] Résistance convection interne de l'échangeur EFD
                    R_convHXefd_out[i,0]=(h_convHXefd_out[i,0]*np.pi*paramHXefd.Do*paramHXefd.L)**-1;  # [K/W] Résistance convection externe de l'échangeur EFD
                    R_HXefd[i,0] = R_convHXefd_in[i,0] + R_condHXecd_wall + R_convHXefd_out[i,0];     # [K/W] Résistance totale de l'échangeur EFD
        ...
        #¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
        A = np.ones([paramRes2.nb_y-1,1])*(-d2 - d3[0:-1]*M_debit_A);                                                               # Vecteur de la diagonale supérieure de longueur N-1
        D = np.ones([paramRes2.nb_y,1])*(d1 + d2*(2 - M_topbott) + d3*M_debit_D + d4 + d5*M_topbott);                                  # Vecteur de la diagonale centrale de longueur N
        C1 = np.ones([paramRes2.nb_y,1])*(d3*M_debit_C*paramRes2.Tink_EC + d4*data.Tairk + d5*M_topbott*data.Tairk - q_EFD);          # Vecteur des constantes C (indépendante du temps m)
        C = np.ones([paramRes2.nb_y,1])*(d1*Res2results.Tk[:,[m-1]]) + C1;                                                                # Vecteur des constantes C (dépendante du temps m)
...
print('La température initial du réservoir est {0}°C.\n'.format(paramRes2.Ti))
...
print('Masse thermique du Cuivre de l''échangeur EFD = {0:.2f} [kJ/K]'.format(data.rho_cu*Volume_cuivre*data.cp_cu*1e-3))
