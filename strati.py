...
#����������������������������
data.theta = -np.pi/2;               # [radians] Inclinaison de la conduite par rapport � la verticale (-pi/2 --> horizontal, 0 --> vertical)
...
paramRes2.y_num = np.arange(paramRes2.dy/2,paramRes2.yi,paramRes2.dy);   # [m] Vecteur de position de chacun des noeuds

# Res2 - Flow parameters
#
paramRes2.m_in = 0.13;                         # [kg/s] D�bit de la pompe 
...
#
paramHXefd.m_in = 0.5;                                       # [kg/s] D�bit de consommation d'eau chaude domestique
paramHXefd.intlet = paramRes2.nb_y;                        # [-] Num�ro du noeud entr�e (nb_y --> bas du r�servoir)
paramHXefd.outlet = 0;                                     # [-] Num�ro du noeud sortie (0--> haut du reservoir)
...
# Res2
#������������������
paramRes2.Veau = paramRes2.Vres -(paramHXefd.Aext*paramHXefd.Ltot);                          # [m^3] Volume d'eau dans le r�servoir --> Calcul Volume EC = (Volume interne du r�servoir - Volume des �changeurs de chaleurs)
paramRes2.dV = paramRes2.Veau/paramRes2.nb_y;                                                # [m^3] Volume d'un noeud du r�servoir de stockage
paramRes2.deltak = data.kss*(np.pi*paramRes2.De**2/4-paramRes2.As)/paramRes2.As;             # De-stratification due au transfert de chaleur par conduction axiale dans la paroi du r�servoir
...
M_topbott[0] = 1;        # Identification du noeud du haut (Condition aux fronti�res)
M_topbott[-1] = 1;      # Identification du noeud du bas (Condition aux fronti�res)
...
M_debit_C[paramRes2.intlet_EC-1] = 1; # Identification des noeuds de d�bit EC pour la diagonale C (Matrice des coefficients)
...
#
Res2results.Tk = np.zeros([paramRes2.nb_y,nb_t]) + paramRes2.Tik;   # [K] Initialisation de la matrice de temp�rature EC (ligne = #noeud et colonne = pas de temps)

# Initialisation des vecteurs de solutions de l'�changeur EFD
#
HXefdresults.Tink = np.zeros([paramRes2.nb_y,nb_t]);                # [K] Initialisation de la temp�rature dans l'�changeur EFD � l'entr�e de chaque noeud du r�servoir
HXefdresults.Tink[:,[0]] = Res2results.Tk[:,[0]]*M_HXefd;  # [K] Initialisation de la temp�rature dans l'�changeur EFD (Conditions initiales --> Time step #1)
HXefdresults.Tink[paramHXefd.intlet-1,np.arange(1,nb_t,1)] = paramHXefd.Tink;      # [K] Initialisation de la temp�rature � l'entr�e de l'�changeur d'EFD (Boundary condition)

HXefdresults.Toutk = np.copy(HXefdresults.Tink);                        # [K] Initialisation de la temp�rature dans l'�changeur EFD � la sortie de chaque noeud du r�servoir

HXefdresults.Ts_outk = np.zeros([paramRes2.nb_y,nb_t]);             # [K] Initialisation de la temp�rature de surface externe de l'�changeur d'EFD
HXefdresults.Ts_outk[:,[1]] = Res2results.Tk[:,[0]]*M_HXefd;      # [K] Initialisation de la temp�rature de surface externe de l'�changeur d'EFD (Conditions initiales --> Time step #1)
...
#
h_convHXefd_in = np.zeros([paramRes2.nb_y,1]);       # [W/(m^2) K] Convection heat transfer coefficient inside �changeur EFD (convection forc�e)
h_convHXefd_out = np.zeros([paramRes2.nb_y,1]);      # [W/(m^2) K] Convection heat transfer coefficient outside �changeur EFD (convection naturelle)
...
R_condHXecd_wall = np.log(paramHXefd.Do/paramHXefd.Di)/(2*np.pi*data.kcu*paramHXefd.L);      # [K/W] Calcul de la r�sistance thermique totale au travers de la paroi de l'�changeur EFD

d2 = paramRes2.As*(data.kss + paramRes2.deltak)/paramRes2.dy;                          # Terme constant de la diagonale D (Matrice des coefficients)
B = np.ones([paramRes2.nb_y-1,1])*(-d2);                                                    # Vecteur de la diagonale inf�rieure B de longueur (Nb_y-1) (Matrice des coefficients)
...
for m in range(1,nb_t,1):
    ...
    while erreurTres2[iterTres2] > data.critere:
        ...
        #�����������������������������������
        for i in range(paramRes2.nb_y+2):  # La boucle se fait sur nb_y+2 afin de calculer les pertes thermiques des parois sur les nb_y noeuds du r�servoir ainsi que pour le dessus et le dessous du r�servoir (conditions aux fronti�res)
            ...
            #
            propriFluid_EC.rho_inf[i,0] = properties.eau_prop('vf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2)**-1;     # [kg/m3] Density
            propriFluid_EC.k_inf[i,0] = properties.eau_prop('kf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2);          # [W/(m K)] Thermal conductivity
            propriFluid_EC.mu_inf[i,0] = properties.eau_prop('muf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2);        # [Ns/m^2] Dynamic viscosity
            propriFluid_EC.Pr_inf[i,0] = properties.eau_prop('Prf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2);        # [-] Prandtl Number
            propriFluid_EC.beta_inf[i,0] = properties.eau_prop('betaf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2);    # [1/K]  Volumetric expansivity (beta)
            propriFluid_EC.Cp_inf[i,0] = properties.eau_prop('Cpf',(Res2results.Tk[j,m]+Tsk_in[i,0])/2);        # [J/(kg K)] Heat capacity
            propriFluid_EC.alpha_inf[i,0] = propriFluid_EC.k_inf[i,0]/propriFluid_EC.rho_inf[i,0]/propriFluid_EC.Cp_inf[i,0]; # [m^2/s] Thermal diffusivity
            ...
            if i == 0:# Pour le haut du r�servoir
                h_convEC[i,0] = Churchill_Chu_plate(paramRes2.Di,propriFluid_EC.rho_inf[i,0],
                         propriFluid_EC.Pr_inf[i,0],propriFluid_EC.k_inf[i,0],propriFluid_EC.beta_inf[i,0],
                             propriFluid_EC.alpha_inf[i,0],propriFluid_EC.mu_inf[i,0],Tsk_in[i,0],Res2results.Tk[j,1],3); # [W/(m^2) K] Convection heat transfer coefficient in EC
                ...
                h_convEC[i,0] = Churchill_Chu_plate(paramRes2.yi,propriFluid_EC.rho_inf[i,0],
                         propriFluid_EC.Pr_inf[i,0],propriFluid_EC.k_inf[i,0],propriFluid_EC.beta_inf[i,0],
                             propriFluid_EC.alpha_inf[i,0],propriFluid_EC.mu_inf[i,0],Tsk_in[i,0],Res2results.Tk[j,1],1); # [W/(m^2) K] Convection heat transfer coefficient in EC
            ...
            propriAir.k[i,0] = properties.air_prop('k',(data.Tairk+Tsk_out[i,0])/2);      # [W/(m K)] Thermal conductivity
            propriAir.mu[i,0] = properties.air_prop('mu',(data.Tairk+Tsk_out[i,0])/2);    # [Ns/m^2] Dynamic viscosity
            ...
            propriAir.alpha[i,0] = properties.air_prop('al',(data.Tairk+Tsk_out[i,0])/2); # [m^2/s] Thermal diffusivity
            ...
            if i == 0: # Pour le dessus du r�servoir
                h_convair[i,0] = Churchill_Chu_plate(paramRes2.De,propriAir.rho[i,0],
                         propriAir.Pr[i,0],propriAir.k[i,0],propriAir.beta[i,0],
                             propriAir.alpha[i,0],propriAir.mu[i,0],Tsk_out[i,0],data.Tairk,2); # [W/(m^2) K] Convection heat transfer coefficient
            elif i == (paramRes2.nb_y+1): # Pour le dessous du r�servoir
                h_convair[i,0] = Churchill_Chu_plate(paramRes2.De,propriAir.rho[i,0],
                         propriAir.Pr[i,0],propriAir.k[i,0],propriAir.beta[i,0],
                             propriAir.alpha[i,0],propriAir.mu[i,0],Tsk_out[i,0],data.Tairk,3); # [W/(m^2) K] Convection heat transfer coefficient
            else: # Pour les c�t�s
                h_convair[i,0] = Churchill_Chu_plate(paramRes2.ye,propriAir.rho[i,0],
                         propriAir.Pr[i,0],propriAir.k[i,0],propriAir.beta[i,0],
                             propriAir.alpha[i,0],propriAir.mu[i,0],Tsk_out[i,0],data.Tairk,1); # [W/(m^2) K] Convection heat transfer coefficient
        ...
        Rpp_pertestopbott[0] = Rpp_convEC[0] + Rpp_condtanktop + Rpp_condiso + Rpp_convair[0];        # [K (m^2)/W] R�sistance thermique r�sultante pour le dessus
        Rpp_pertestopbott[-1] = Rpp_convEC[-1] + Rpp_condtankbott + Rpp_condiso + Rpp_convair[-1]; # [K (m^2)/W] R�sistance thermique r�sultante pour le dessous
        Rpp_pertesside = Rpp_convEC[1:-1] + Rpp_condtankside + Rpp_condiso + Rpp_convair[1:-1]; # [K (m^2)/W] R�sistance thermique r�sultante pour les c�t�s
        Rpp_pertes = np.vstack((Rpp_pertestopbott[0],Rpp_pertesside,Rpp_pertestopbott[-1]));  # [K (m^2)/W] Vecteur des r�sistances thermiques totales pour le dessus, dessous et c�t�s
        ...
        for ii in range(paramHXefd.intlet,paramHXefd.outlet,-1):
            ...
            if paramHXefd.m_in > 0:
                ...
                while erreurTout[iterToutHX] > data.critere:
                    ...
                    TmoyEFD[i,0] = (HXefdresults.Tink[i,m] + HXefdresults.Toutk[i,m])/2; # Calcul de la temp�rature moyenne entre la temp�rature d'entr�e et la temp�rature de sortie de l'�changeur
                    
                    # Calcul des propri�t�s de l'eau dans le r�servoir EC
                    #
                    propriHXefd_out.rho_inf[i,0] = properties.eau_prop('vf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2)**-1;                         # [kg/m3] Density
                    propriHXefd_out.k_inf[i,0] = properties.eau_prop('kf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2);                                 # [W/(m K)] Thermal conductivity
                    propriHXefd_out.mu_inf[i,0] = properties.eau_prop('muf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2);                               # [Ns/m^2] Dynamic viscosity
                    propriHXefd_out.Pr_inf[i,0] = properties.eau_prop('Prf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2);                               # [-] Prandtl Number
                    propriHXefd_out.beta_inf[i,0] = properties.eau_prop('betaf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2);                           # [1/K]  Volumetric expansivity (beta)
                    propriHXefd_out.Cp_inf[i,0] = properties.eau_prop('Cpf',(TmoyEFD[i,0] + Res2results.Tk[i,m])/2);                               # [J/(kg K)] Heat capacity
                    propriHXefd_out.alpha_inf[i,0] = propriHXefd_out.k_inf[i,0]/(propriHXefd_out.rho_inf[i,0]*propriHXefd_out.Cp_inf[i,0]); # [m^2/s] Thermal diffusivity
                    ...
                    h_convHXefd_out[i,0] = Churchill_Chu(paramHXefd.Do,propriHXefd_out.rho_inf[i,0],
                           propriHXefd_out.Pr_inf[i,0],propriHXefd_out.k_inf[i,0],propriHXefd_out.beta_inf[i,0],
                               propriHXefd_out.alpha_inf[i,0],propriHXefd_out.mu_inf[i,0],HXefdresults.Ts_outk[i,m],Res2results.Tk[i,0]); # [W/(m^2 K)] Convection heat transfer coefficient
                    ...
                    propriHXefd_in.k_inf[i,0] = properties.eau_prop('kf',TmoyEFD[i,0]);                                                      # [W/(m K)] Thermal conductivity
                    propriHXefd_in.mu_inf[i,0] = properties.eau_prop('muf',TmoyEFD[i,0]);                                                    # [Ns/m^2] Dynamic viscosity
                    propriHXefd_in.Pr_inf[i,0] = properties.eau_prop('Prf',TmoyEFD[i,0]);                                                    # [-] Prandtl Number
                    propriHXefd_in.beta_inf[i,0] = properties.eau_prop('betaf',TmoyEFD[i,0]);                                                # [1/K]  Volumetric expansivity (beta)
                    propriHXefd_in.Cp_inf[i,0] = properties.eau_prop('Cpf',TmoyEFD[i,0]);                                                    # [J/(kg K)] Heat capacity
                    propriHXefd_in.alpha_inf[i,0] = propriHXefd_in.k_inf[i,0]/(propriHXefd_in.rho_inf[i,0]*propriHXefd_in.Cp_inf[i,0]); # [m^2/s] Thermal diffusivity
                    
                    # Calul du coefficient de convection forc�e � l'int�rieur de l'�changeur EFD
                    #
                    h_convHXefd_in[i,0] = Dittus_Boelter_mod(paramHXefd.Di,paramHXefd.D,paramHXefd.pitch,paramHXefd.m_in,propriHXefd_in.Cp_inf[i,0],
                        propriHXefd_in.k_inf[i,0],propriHXefd_in.mu_inf[i,0],propriHXefd_in.Pr_inf[i,0]);      # [W/(m^2 K)] Convection heat transfer coefficient
                    
                    # Calcul des r�sistances des flux de chaleur entre l'�changeur ECD et le r�servoir
                    #
                    R_convHXefd_in[i,0]=(h_convHXefd_in[i,0]*np.pi*paramHXefd.Di*paramHXefd.L)**-1;    # [K/W] R�sistance convection interne de l'�changeur EFD
                    R_convHXefd_out[i,0]=(h_convHXefd_out[i,0]*np.pi*paramHXefd.Do*paramHXefd.L)**-1;  # [K/W] R�sistance convection externe de l'�changeur EFD
                    R_HXefd[i,0] = R_convHXefd_in[i,0] + R_condHXecd_wall + R_convHXefd_out[i,0];     # [K/W] R�sistance totale de l'�changeur EFD
                    
                    # Recalcul de la temp�rature de sortie de l'�changeur pour le noeud du r�servoir i
                    #
                    HXefdresults.Toutk[i,m] = Res2results.Tk[i,m] + (HXefdresults.Tink[i,m] - Res2results.Tk[i,m])*np.exp((-R_HXefd[i,0]*paramHXefd.m_in*propriHXefd_in.Cp_inf[i,0])**-1);
                    ...
                    #
                    HXefdresults.Ts_outk[i,m] = TmoyEFD[i,0] + q_EFD[i,0]*(R_convHXefd_in[i,0] + R_condHXecd_wall); # [K]
        ...
        #����������������������������������������������������
        A = np.ones([paramRes2.nb_y-1,1])*(-d2 - d3[0:-1]*M_debit_A);                                                               # Vecteur de la diagonale sup�rieure de longueur N-1
        D = np.ones([paramRes2.nb_y,1])*(d1 + d2*(2 - M_topbott) + d3*M_debit_D + d4 + d5*M_topbott);                                  # Vecteur de la diagonale centrale de longueur N
        C1 = np.ones([paramRes2.nb_y,1])*(d3*M_debit_C*paramRes2.Tink_EC + d4*data.Tairk + d5*M_topbott*data.Tairk - q_EFD);          # Vecteur des constantes C (ind�pendante du temps m)
        C = np.ones([paramRes2.nb_y,1])*(d1*Res2results.Tk[:,[m-1]]) + C1;                                                                # Vecteur des constantes C (d�pendante du temps m)
...
print('Masse thermique du Cuivre de l''�changeur EFD = {0:.2f} [kJ/K]'.format(data.rho_cu*Volume_cuivre*data.cp_cu*1e-3))
